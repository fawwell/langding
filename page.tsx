...
export default function V2MainPage() {
    ...
    useEffect(() => {
        // 1. 스크롤 애니메이션 (IntersectionObserver 활용)
        ...
        document.querySelectorAll('.hero-stats, .school-stats, .jelly-chart-section').forEach(el => countUpObserver.observe(el));

        // 3. 모바일 슬라이더 세팅 (initMobileSliders)
        ...
        grids.forEach(grid => {
            ...
            const slideNext = () => {
                ...
                const cardWidth = (grid.children[0] as HTMLElement).offsetWidth + 15;
    ...
    }, [activePage, activePhysicalSub]); // 페이지 변경 시 애니메이션 재관찰
    ...
    const submitProposalForm = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        requests.unshift(data); // 최신 데이터 맨 앞으로 (script.js 로직)
        localStorage.setItem('faww_requests', JSON.stringify(requests));
        
        showToast('✅ 제안서 요청이 성공적으로 접수되었습니다.<br><span style="font-size:13px; color:#aaa; font-weight:normal; margin-top:5px; display:inline-block;">(admin 페이지에서 접수 내역을 확인해 보세요!)</span>');
        closeModal();
        form.reset();
    };
    ...
    const showQuizResult = (type: string) => {
        if (type === 'eap1') {
            setQuizResultTitle("스마트 AI 스캐닝 + 1:1 수기 케어");
            setQuizResultDesc("근골격계 질환의 원인을 정확히 찾고, 원조 전문가가 현장에서 직접 케어하여 즉각적인 업무 효율을 높입니다.");
        } else if (type === 'eap2') {
            setQuizResultTitle("1:1 프리미엄 릴렉싱 케어");
            setQuizResultDesc("신체의 굳은 긴장을 이완시켜 교감신경을 안정화하고 정신적 번아웃을 예방하는 최적의 솔루션입니다.");
        } else if (type === 'eap3') {
            setQuizResultTitle("오피스 단체 스트레칭 + 특강");
            setQuizResultDesc("사무실 의자를 활용한 실습과 거북목 교정 특강을 통해 다함께 참여하는 건강 문화를 만듭니다.");
        } else if (type === 'sch1') {
            setQuizResultTitle("학교 전용 3D AI 체형 검진 시스템");
            setQuizResultDesc("모아레 및 척추 분석 기능을 통해 학생들의 성장 밸런스를 측정하고 학부모용 상세 리포트를 제공합니다.");
        } else if (type === 'sch2') {
            setQuizResultTitle("학생 기능성 그룹 트레이닝");
            setQuizResultDesc("체형 분석을 기반으로 성장기 학생들에게 꼭 필요한 맞춤형 교정 운동과 스트레칭을 지도합니다.");
        }
        setQuizStep(3);
    }
                ...
                <section className="testimonials reveal">
                    <div className="container">
                        ...
                        <div className="swiper reviewSwiper" style={{ marginTop: '40px', padding: '20px 0' }}>
                            <div className="swiper-wrapper" id="review-wrapper" style={{ display: 'flex', overflowX: 'auto', gap: '20px' }}>
                                {reviewsData.map((rev, index) => {
                                    const isVisible = reviewFilter === 'all' || reviewFilter === rev.type;
                                    return (
                                    <div key={index} className={isVisible ? 'swiper-slide' : ''} style={{ display: isVisible ? '' : 'none', minWidth: '300px', flex: '0 0 auto' }}>
                                ...
                                <div className="proposal-module-group" style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
                                    <div className="proposal-block">
                                        <div className="proposal-block-header" onClick={toggleSubModulesList}>
                                            <span>▪ PART 1. 진단 (스마트 AI 체형분석)</span> <span className="toggle-icon">▼</span>
                                        </div>
                                    ...
                                    <div className="proposal-block">
                                        <div className="proposal-block-header" onClick={toggleSubModulesList}>
                                            <span>▪ PART 2. 케어 (1:1 피지컬 케어)</span> <span className="toggle-icon">▼</span>
                                        </div>
                                        <div className="sub-modules" style={{ display: 'none', padding: '15px', borderTop: '1px solid #eaeaea', background: '#fafafa', flexDirection: 'column', gap: '10px' }}>
                                    ...
                                    <div className="proposal-block">
                                        <div className="proposal-block-header" onClick={toggleSubModulesList}>
                                            <span>▪ PART 3. 실습 (단체 운동 프로그램)</span> <span className="toggle-icon">▼</span>
                                        </div>
                                        <div className="sub-modules" style={{ display: 'none', padding: '15px', borderTop: '1px solid #eaeaea', background: '#fafafa', flexDirection: 'column', gap: '10px' }}>
                                    ...
                                    <div className="proposal-block">
                                        <div className="proposal-block-header" onClick={toggleSubModulesList}>
                                            <span>▪ PART 4. 교육 (강의 프로그램)</span> <span className="toggle-icon">▼</span>
                                        </div>
                                        <div className="sub-modules" style={{ display: 'none', padding: '15px', borderTop: '1px solid #eaeaea', background: '#fafafa', flexDirection: 'column', gap: '10px' }}>
